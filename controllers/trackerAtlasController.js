...
const trackerAtlasController = {
    getAll: async (req, res) => {
        try {
            const trackers = await TrackerAtlas.find();
            //send response
            res.json(trackers);
        } catch (error) {
    ...
    getById: async (req, res) => {
        try {
            const tracker = await TrackerAtlas.findById(req.params.id);
    ...
    delete: async (req, res) => {
        try {
            ...
            await TrackerAtlas.findOneAndDelete({ sender_id });
            //send response
    ...
    getNormal: async(req, res) => {
        try {
            ...
            const normalConversations = await TrackerAtlas.find({
                events: {
                    $not: {
    ...
    getFallback: async (req, res) => {
        try {
            const fallbackConversations = await TrackerAtlas.find({
                events: {
                    $elemMatch: { name: 'action_default_fallback' },
                },
            });
...
module.exports = trackerAtlasController;